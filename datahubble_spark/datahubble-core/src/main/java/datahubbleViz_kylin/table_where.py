...
from get_date_seq import get_date_between
            ...
            for column_id in range(self.column_num):
                ...
                if f.type == Type.numerical or f.type == Type.temporal:
                    if f.type == Type.temporal:
                        sql = ('select {0} from {1}'+ " where 1=1 " + self.instance.filter + ' group by {0}').format(self.names[column_id], self.instance.tale_name)
                        results = kylin_util.executeQuery(sql)
                        dates = list(map(lambda x: list(x)[-1], results))
                        f.min, f.max = min(dates), max(dates)
                        f.minmin = f.min
                    else:
                        sql = 'select min({0}), max({0}) from {1}'.format(self.names[column_id], self.instance.tale_name)
                        sql += " where 1=1 " + self.instance.filter
                        results = kylin_util.executeQuery(sql)
                ...
                if f.type == Type.temporal:
                    ...
                    sql = ''
                    if interval == 'YEAR':
                        sql = 'select YEAR({0}) from {1}'+ " where 1=1 " + self.instance.filter + ' group by YEAR({0})'
                    elif interval == 'MONTH':
                        sql = 'select MONTH({0}) from {1}'+ " where 1=1 " + self.instance.filter + ' group by MONTH({0})'
                    elif interval == 'DAY':
                        sql = 'select DAYOFMONTH({0}) from {1}'+ " where 1=1 " + self.instance.filter + ' group by DAYOFMONTH({0})'
                    elif interval == 'HOUR':
                        sql = 'select HOUR({0}) from {1}'+ " where 1=1 " + self.instance.filter + ' group by HOUR({0})'
                    elif interval == 'MINUTE':
                        sql = 'select MINUTE({0}) from {1}'+ " where 1=1 " + self.instance.filter + ' group by MINUTE({0})'
                    elif interval == 'SECOND':
                        sql = 'select SECOND({0}) from {1}'+ " where 1=1 " + self.instance.filter + ' group by SECOND({0})'
    ...
    def dealWithGroup(self, column_id, begin, end, get_head, get_data, filter=None):
        ...
        if get_data:
            if filter:
                ...
                sql=sql[:-1]
                sql += " from {} ".format(self.instance.tale_name) + \
                       ' where 1=1 ' + self.instance.filter + \
                       " group by {}".format(self.names[column_id])
                results = kylin_util.executeQuery(sql)

            if self.types[column_id] == Type.temporal:
                dates = list(map(lambda x: list(x)[-1], results))
                min_date = min(dates)
                max_date = max(dates)
                dates = get_date_between(min_date, max_date)
                new_result = []
                ...
                for item in results:
                    it = list(item)
                    if it[-1] in dates:
                        new_result[dates.index(it[-1])] = it
                for item in new_result:
                    new_table.D.append(list(item))
            else:
                distinct = map(lambda item: item[0], self.features[column_id].distinct_values)
                m = dict(map(lambda item: [item[-1], item[:-1]], results))
                size = len(m.values()[0])
                temp = []
                for item in distinct:
                    if item in m:
                        i = list(m[item][0:size])
                    else:
            ...
            new_table.tuple_num = len(new_table.D)
        return new_table
    ...
    def dealWithTable(self):
        ...
        for i in range(self.column_num):
            if self.features[i].ratio < 1.0 and (self.types[i] == Type.temporal or (
                    self.types[i] == Type.categorical and self.features[i].distinct <= 30)):
