...
class Table(object):
    def __init__(self, instance, transformed, describe1, describe2):
        ...
        self.column_num = self.tuple_num = self.view_num =self.countview_num =0
        self.names = []
        self.types = []
        self.origins = []
        self.features = []
    ...
    def getNumericalBins(self, f):
        ...
        if maxNum<=50:
            ...
            for i in range(50):
                n_point = (maxNum - minNum) / 50 * i + minNum
                n1_point = (maxNum - minNum) / 50 * (i + 1) + minNum
                bins.append([str(n_point)+','+ str(n1_point)])
        f.interval_bins = bins
        f.bin_num = len(bins)
        f.interval = 'Numerical'
        ...
        if not self.transformed:
            for column_id in range(self.column_num):
                ...
                if f.type == Type.numerical:
                    self.getNumericalBins(f)
                self.features.append(f)
        else:
            for column_id in range(self.column_num):
                f = Features(self.names[column_id], self.types[column_id], self.origins[column_id])
                # calculate min,max for count
                if f.type == Type.numerical:
    ...
    def dealWithPNBin(self, column_id, begin, end, get_head, get_data):
        ...
        if get_data:
            d = {}
            for i in range(self.features[column_id].bin_num):
                for i in range(begin, end):
                    if self.D[i][column_id] in range(self.features[column_id].bin_num):
                        new_table.D[0][1] += 1
                    else:
                        new_table.D[1][1] += 1
        return new_table

    def dealWithNumericalBins(self, column_id, begin, end, get_head, get_data):
        new_table = Table(self.instance, True, 'BIN ' + self.names[column_id], '')
        if get_head:
            new_table.column_num =2
            new_table.tuple_num = self.features[column_id].bin_num
            new_table.names.extend([self.names[column_id], 'CNT(' + self.names[column_id] + ')'])
            new_table.types.extend([Type.categorical, Type.numerical])
            new_table.origins.extend([column_id, column_id])
        if get_data:
            d = []
            for i in range(0, self.features[column_id].bin_num):
                d.append(self.features[column_id].interval_bins[i])
            for i in range(self.features[column_id].bin_num):
                count = 0
                for j in range(begin, end):
                    interval=(self.features[column_id].interval_bins[i][0]).split(",")
                    if float(interval[0]) <= self.D[j][column_id] <= float(interval[1]):
                        count += 1
                d[i].append(count)
                new_table.D = d
        return new_table
    ...
    def dealWithTable(self):
        ...
        if self.transformed:
            return new_tables
        # columns to transformed (group or bin and calculate count,sum,avg)
        for i in range(self.column_num):
            ...
            if self.features[i].ratio < 1.0 and (self.types[i] == Type.temporal or self.types[i] == Type.categorical) :
                new_tables.append(self.dealWithGroup(i, 0, self.tuple_num, True, True))
            ...
            # flights数据表进入dealWithPNBin函数，因为new_table.D为空报错，但这之前理应是有相关赋值代码的（但不知道是啥），所以就注释掉啦～
            ...
            #     new_tables.append(self.dealWithNumericalBins(i, 0, self.tuple_num, True, True))
