...
class Instance(object):
    ...
    def singleCol_to_chart(self):
        cw=single_colum_chart.singleCol_to_chart(self)
        return cw

    def getweights(self):
        weights = [0 for i in range(self.column_num)]
        for table in self.tables:
            for view in table.views:
                weights[view.fx.origin] += 1
                weights[view.fy.origin] += 1
                if view.z_id != -1:
                    weights[view.z_id] += 1
        for i in range(self.column_num):
            weights[i] = 1.0 * weights[i] / self.view_num
        return weights
